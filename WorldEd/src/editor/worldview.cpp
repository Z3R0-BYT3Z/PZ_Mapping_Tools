...
void WorldView::mouseMoveEvent(QMouseEvent *event)
{
    ...
    const int cellSize = scene()->world()->cellSize();
    emit statusBarCoordinatesChanged(cellPos.x() * cellSize,
                                     cellPos.y() * cellSize);

    BaseGraphicsView::mouseMoveEvent(event);
}

void WorldView::drawForeground(QPainter *painter, const QRectF &rect)
{
                             ...
                             QWidget *)
{
    ...
    const QRect worldBounds = mScene->world()->bounds();
    for (int y = worldBounds.top(); y <= worldBounds.bottom(); ++y) {
        for (int x = worldBounds.left(); x <= worldBounds.right(); ++x) {
            if (WorldCellItem *cellItem = mScene->itemForCell(x, y))
                cellItem->paintThumbnails(painter);
        }
    }
