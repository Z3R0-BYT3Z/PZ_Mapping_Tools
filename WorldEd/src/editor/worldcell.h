...
public:
    ...
    WorldCellContents(WorldCell *cell, bool takeOwnership)
    {
        ...
        if (takeOwnership) {
            ...
            cell->mLots.clear();
            cell->mObjects.clear();
            cell->mInGameMap.mFeatures.clear();
        } else {
            ...
            cloneInGameMapFeatures(cell->mInGameMap.mFeatures, newOwner);
        }
    }
    ...
    void setCellContents(WorldCell *cell, bool transferOwnership = true)
    {
        ...
        if (transferOwnership) {
            ...
            for (InGameMapFeature *feature : std::as_const(mInGameMapFeatures)) {
                feature->mOwner = &cell->mInGameMap;
            ...
            mLots.clear();
            mObjects.clear();
            mInGameMapFeatures.clear();
        } else {
            cell->mProperties = mProperties.clone();
            cell->mLots = mLots.clone(cell);
            cell->mObjects = mObjects.clone(cell);
            for (InGameMapFeature *feature : std::as_const(mInGameMapFeatures)) {
                InGameMapFeature *copy = new InGameMapFeature(&cell->mInGameMap);
                copy->mGeometry = feature->mGeometry;
                copy->mProperties = feature->mProperties;
                cell->mInGameMap.mFeatures += copy;
            }
        }
    }
    ...
    const QList<InGameMapFeature*> &inGameMapFeatures() const
    { return mInGameMapFeatures; }
...
private:
    void cloneInGameMapFeatures(const QList<InGameMapFeature*> &features,
                                WorldCell *newOwner)
    {
        for (InGameMapFeature *feature : features) {
            InGameMapFeature *copy = new InGameMapFeature(&newOwner->mInGameMap);
            copy->mGeometry = feature->mGeometry;
            copy->mProperties = feature->mProperties;
            mInGameMapFeatures += copy;
        }
    }
