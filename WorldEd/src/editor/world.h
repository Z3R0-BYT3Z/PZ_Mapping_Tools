...
{
public:
    World(int width, int height, WorldGridFormat gridFormat = WorldGridFormat::Legacy300);
    ~World();
    ...
    WorldGridFormat gridFormat() const { return mGridFormat; }
    WorldGeometry geometry() const { return WorldGeometry::forFormat(mGridFormat); }
    int cellSize() const { return geometry().cellSize; }
