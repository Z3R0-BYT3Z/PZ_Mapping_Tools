...
World::World(int width, int height, WorldGridFormat gridFormat)
    : mWidth(width)
    , mHeight(height)
    , mGridFormat(gridFormat)
...
WorldCellLotList World::getLotsOverlappingCellBounds(int cellX, int cellY, const QRect &ignoreCells)
{
    ...
    QRect cellRect(cellX * size, cellY * size, size, size);
...
RoadList World::roadsInRect(const QRect &bounds)
{
    RoadList result;
    result.reserve(mRoads.size());
    for (Road *road : std::as_const(mRoads)) {
