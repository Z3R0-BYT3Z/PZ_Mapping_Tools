...
QRect WorldCell::bounds() const
{
    const int size = mWorld ? mWorld->cellSize() : 300;
...
void WorldCell::getLotsOverlappingCellBounds(WorldCellLotList &result) const
{
    ...
    for (WorldCellLot *lot : lots()) {
        if (!QRect(0, 0, size, size).contains(lot->bounds())) {
            result += lot;
        }
    }
}
...
{
    for (WorldCellLot *lot : lots()) {
        QRect lotBounds = lot->bounds().translated(bounds().topLeft());
