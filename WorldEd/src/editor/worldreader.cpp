...
private:
    World *readWorld()
    {
        ...
        WorldGridFormat gridFormat;
        const QString gridFormatName = atts.value(QLatin1String("cellFormat")).toString();
        if (!worldGridFormatFromName(gridFormatName, gridFormat)) {
            xml.raiseError(tr("Unsupported world cell format '%1'.").arg(gridFormatName));
            return nullptr;
        }

        mWorld = new World(width, height, gridFormat);
    ...
    void readGenerateLots()
    {
        ...
        while (xml.readNextStartElement()) {
            if (xml.name() == QLatin1String("exportdir")) {
                ...
                xml.skipCurrentElement();
            } else if (xml.name() == QLatin1String("BiomeMap")) {
                QString path = xml.attributes().value(QLatin1String("path")).toString();
                settings.biomeMap = resolveReference(path, mPath);
                xml.skipCurrentElement();
    ...
    void readLuaSettings()
    {
        ...
        while (xml.readNextStartElement()) {
            if (xml.name() == QLatin1String("spawnPointsFile")) {
                ...
                xml.skipCurrentElement();
            } else if (xml.name() == QLatin1String("worldObjectsFile")) {
                QString path = xml.attributes().value(QLatin1String("path")).toString();
