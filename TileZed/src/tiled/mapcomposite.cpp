...
void CompositeLayerGroup::setUseImageBlack(int x, int y, bool value)
{
    ...
    if (mUseImageBlackBounds != bounds) {
        ...
        if (!mUseImageBlackBounds.isEmpty()) {
            ...
            for (int gridY = overlap.y(); gridY <= overlap.bottom(); ++gridY) {
                for (int gridX = overlap.x(); gridX <= overlap.right(); ++gridX) {
                    grid.setBit(gridX - bounds.x() +
                                (gridY - bounds.y()) * bounds.width(),
                                mUseImageBlack.testBit(
                                    gridX - mUseImageBlackBounds.x() +
                                    (gridY - mUseImageBlackBounds.y()) *
                                    mUseImageBlackBounds.width()));
                }
            }
        }
        mUseImageBlack = grid;
    ...
    mUseImageBlack.setBit(x - bounds.x() +
                          (y - bounds.y()) * bounds.width(), value);
}
...
bool CompositeLayerGroup::useImageBlack(int x, int y) const
{
    ...
    return mUseImageBlack.testBit(
                x - mUseImageBlackBounds.x() +
                (y - mUseImageBlackBounds.y()) *
                mUseImageBlackBounds.width());
}
