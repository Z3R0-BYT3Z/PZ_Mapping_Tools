                                ...
                                const QModelIndex &index) const
{
    ...
    bool prevRowSelected = index.row() > 0 &&
            mView->selectionModel()->isRowSelected(index.row() - 1, QModelIndex());
    ...
    if (namedTile != BuildingEditor::BuildingTilesMgr::instance()->noneTiledTile()) {
        const QString tileName = BuildingEditor::BuildingTilesMgr::nameForTile(namedTile);
        const QString elidedName = fm.elidedText(tileName, Qt::ElideMiddle,
                                                 option.rect.width() - extra * 2);
        painter->setPen(option.palette.color(QPalette::Text));
        painter->drawText(option.rect.adjusted(extra, 0, -extra, -extra),
                          Qt::AlignHCenter | Qt::AlignBottom, elidedName);
    }
                                    ...
                                    const QModelIndex &index) const
{
    ...
    const int labelHeight = fm.lineSpacing();
    if (!m->tileAt(index))
        return QSize(64 * zoom + extra, 2 + labelHeight + 128 * zoom + extra + labelHeight);
    ...
    return QSize(qMax(qMax(tileWidth, mView->maxHeaderWidth()) + extra, viewWidth),
                 2 + labelHeight + qCeil(tileset->tileHeight() * zoom) + extra + labelHeight);
}
