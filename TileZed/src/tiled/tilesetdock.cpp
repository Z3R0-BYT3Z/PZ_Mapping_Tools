...
QIcon tilesetResolutionIcon(const QString &resolution)
{
    if (resolution == QStringLiteral("2x"))
        return QIcon(QStringLiteral(":/images/2x.png"));
    if (resolution == QStringLiteral("1x"))
...
void updateTilesetListItem(QListWidgetItem *item, Tileset *tileset, Map *map)
{
    ...
    const QString resolution = tilesetResolution(tileset);
    ...
    if (tileset->isMissing()) {
        if (inUse) {
            item->setForeground(QBrush(QColor(230, 135, 35)));
            status = QCoreApplication::translate(
                        "TilesetDock", "used by the map, but the image is missing");
        } else {
            item->setForeground(QBrush(QColor(220, 70, 70)));
            status = QCoreApplication::translate(
                        "TilesetDock", "referenced, but the image is missing");
        }
    } else if (inUse) {
        item->setForeground(QBrush(QColor(35, 165, 80)));
        status = QCoreApplication::translate(
    ...
    font.setBold(inUse);
    item->setFont(font);
    item->setIcon(tilesetResolutionIcon(resolution));
    item->setToolTip(QCoreApplication::translate(
                         "TilesetDock", "Status: %1\nUsage: %2\nAvailability: %3\nResolution: %4\nSource: %5")
                     .arg(status,
                          inUse
                          ? QCoreApplication::translate("TilesetDock", "used")
                          : QCoreApplication::translate("TilesetDock", "unused"),
                          tileset->isMissing()
                          ? QCoreApplication::translate("TilesetDock", "missing")
                          : QCoreApplication::translate("TilesetDock", "available"),
                          resolution,
...
void TilesetDock::setMapDocument(MapDocument *mapDocument)
{
    ...
    if (mMapDocument) {
        ...
        if (mTilesetByName.contains(cacheName)) {
            int row = mTilesets.indexOf(mTilesetByName.value(cacheName));
...
void TilesetDock::writeSettings(QSettings &settings)
{
    ...
    if (mSplitter->isVisible() && totalSize > 0) {
        settings.setValue(QLatin1String("splitter1/state"),
                          mSplitter->saveState());
...
void TilesetDock::readSettings(QSettings &settings)
{
    ...
    const QByteArray state =
            settings.value(QLatin1String("splitter1/state")).toByteArray();
    if (!state.isEmpty()) {
        mSplitter->restoreState(state);
    } else {
        QVariant v = settings.value(
                    tr("%1/sizes").arg(mSplitter->objectName()));
...
void TilesetDock::tilePicked(Tile *tile)
{
    ...
    if (mTilesetByName.contains(tile->tileset()->name())) {
        int row = mTilesets.indexOf(mTilesetByName.value(tile->tileset()->name()));
        mTilesetNamesView->setCurrentRow(row);
...
void TilesetDock::tilesetChanged(Tileset *tileset)
{
    ...
    if (mTilesets.contains(tileset)) {
        ...
        if (QListWidgetItem *item = mTilesetNamesView->item(row))
            updateTilesetListItem(item, tileset, mMapDocument->map());
    }
}
...
void TilesetDock::setTilesetNamesList()
{
    ...
    if (mMapDocument && mMapDocument->map()->tilesets().size()) {
        ...
        for (Tileset *ts : mTilesets) {
            ...
            updateTilesetListItem(item, ts, mMapDocument->map());
...
void TilesetDock::refreshTilesetStatus()
{
    if (!mMapDocument)
        return;
