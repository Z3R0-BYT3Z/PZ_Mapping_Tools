...
#ifdef ZOMBOID
QString tileFileName(const Tile *tile)
{
    QString tilesetName = QFileInfo(tile->tileset()->imageSource()).completeBaseName();
    if (tilesetName.isEmpty())
        tilesetName = tile->tileset()->name();
    QString fileName = QStringLiteral("%1_%2").arg(tilesetName).arg(tile->id());
    const QString invalidCharacters = QStringLiteral("<>:\"/\\|?*");
    for (int i = 0; i < fileName.size(); ++i) {
        if (invalidCharacters.contains(fileName.at(i)))
                         ...
                         const QModelIndex &index) const
{
    ...
    if (tile) {
        ...
        const QFontMetrics fm = painter->fontMetrics();
        const int textWidth = qMin(option.rect.width(), fm.horizontalAdvance(idText) + 4);
        const QRect idRect(option.rect.left(), option.rect.top(), textWidth,
                           fm.lineSpacing() + 2);
        painter->fillRect(idRect, QColor(0, 0, 0, 180));
        painter->setPen(Qt::white);
        painter->drawText(idRect.adjusted(2, 0, -1, 0),
                          Qt::AlignLeft | Qt::AlignVCenter, idText);
    }
#endif
}
...
void TilesetView::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    QAction *actionExportSelection = 0;
    if (tile) {
        ...
        actionExportSelection = menu.addAction(
                    ...
                    tr("Export Selection as PNG..."));
        Utils::setThemeIcon(actionExportSelection, "document-export");
        menu.addSeparator();
    }
#else
    if (tile) {
    ...
    if (action && action == actionProperties) {
       // Select this tile to make sure it is clear that only the properties
        ...
        const QString settingsKey = QStringLiteral("TilesetView/ExportDirectory");
        const QString initialDirectory = settings->value(
                    settingsKey,
                    PortableSettings::installPath(QStringLiteral("tiles_export"))).toString();
        const QString outputPath = QFileDialog::getExistingDirectory(
        ...
        for (const QModelIndex &selectedIndex : indexes) {
            Tile *selectedTile = m->tileAt(selectedIndex);
            if (!selectedTile)
                continue;
            const QString outputFile = outputDirectory.filePath(
                        tileFileName(selectedTile) + QStringLiteral(".png"));
            if (!selectedTile->image().save(outputFile, "PNG"))
                failures.append(QDir::toNativeSeparators(outputFile));
        }
        if (!failures.isEmpty()) {
            QMessageBox::warning(this, tr("Export Selected Tiles"),
