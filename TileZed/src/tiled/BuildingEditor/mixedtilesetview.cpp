...
//    const QPixmap tileImage = display.value<QPixmap>();
    ...
    const QFontMetrics idMetrics = painter->fontMetrics();
    const int idWidth = qMin(option.rect.width(), idMetrics.horizontalAdvance(idText) + 4);
    const QRect idRect(option.rect.left(), option.rect.top(), idWidth,
                       idMetrics.lineSpacing() + 2);
                                  ...
                                  const QStringList &labels)
{
    ...
    if (tileset && !tileset->isLoaded() && !tileset->isMissing()) {
        ...
        TilesetManager::instance()->waitForTilesets(QList<Tileset*>() << tileset);
    }
                                 ...
                                 const QStringList &headers)
{
    ...
    mTiles = tiles;
    mUserData = userData;
    mTileset = tiles.isEmpty() || hasMultipleTilesets(tiles)
            ? nullptr : tiles.first()->tileset();
    mTileToItem.clear();
