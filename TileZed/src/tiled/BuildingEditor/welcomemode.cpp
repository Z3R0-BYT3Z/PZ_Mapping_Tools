...
void LinkItem::allowRemove()
{
    ...
    bg->setBrush(QApplication::palette().brush(QPalette::Midlight));
    bg->setPen(Qt::NoPen);
...
void WelcomeMode::linkClickedRemove()
{
    ...
    if (fileInfo.exists()) {
        if (QMessageBox::question(BuildingEditorWindow::instance(), tr("Remove Autosave File"),
            ...
            return;
        QFile autosaveFile(link->filePath());
        if (!autosaveFile.remove())
            QMessageBox::warning(BuildingEditorWindow::instance(), tr("Remove File Failed"),
                                 tr("BuildingEd could not remove the autosave file.\n\n"
                                    "File: %1\n"
                                    "Reason: %2\n\n"
                                    "Check that the file is not open and that "
                                    "you have permission to modify this folder.")
                                 .arg(QDir::toNativeSeparators(link->filePath()),
